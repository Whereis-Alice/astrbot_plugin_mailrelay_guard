...
`v1.3.2` 重构邮件中心为全高阅读工作台：桌面端把文件夹、搜索、筛选和邮件列表集中在左侧窄栏，右侧邮件详情从顶部贯穿到底部，HTML 安全预览占据主要高度。投递信息仍按需展开，预览默认阅读尺寸为 120%，可在 90% 至 145% 间本地调节。此更新不改变 SMTP 投递规则、历史保留策略或内容归档开关。
...
安装并重载 `v1.3.2` 后，在 AstrBot Dashboard 的插件页面中打开 **MailRelay Guard - 邮件中心**。该页面由 AstrBot 的已登录 Dashboard 承载，不会额外启动公开的 Web 服务。
...
为了把正文阅读保持在视觉焦点，投递模式、时间、收件人状态和 SMTP 结果等技术信息默认按需展开，不会长期挤占预览区域。手机端仍会先显示列表；打开邮件后进入详情阅读，并可返回列表继续切换。
