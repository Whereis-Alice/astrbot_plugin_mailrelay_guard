...
`v1.3.1` 将邮件中心调整为阅读优先的布局：桌面端邮件列表收窄为导航，选中邮件的预览占据主要工作区，投递详情改为按需展开；HTML 安全预览新增阅读尺寸调节。此更新不改变 SMTP 投递规则、历史保留策略或内容归档开关。
...
安装并重载 `v1.3.1` 后，在 AstrBot Dashboard 的插件页面中打开 **MailRelay Guard - 邮件中心**。该页面由 AstrBot 的已登录 Dashboard 承载，不会额外启动公开的 Web 服务。
