...
`v1.3.3` 将邮件中心重新设计为阅读器布局：桌面端使用 70px 导航轨，文件夹、搜索、刷新和邮件列表组成紧凑定位栏；筛选和归档说明默认折叠。选中邮件后，右侧阅读器占据绝大多数工作区；点击“专注阅读”可临时收起列表，让预览、纯文本或源码全宽显示。投递信息仍按需展开，预览默认阅读尺寸为 120%，可在 90% 至 145% 间本地调节。此更新不改变 SMTP 投递规则、历史保留策略、内容归档开关或 HTML 预览的安全限制。
...
安装并重载 `v1.3.3` 后，在 AstrBot Dashboard 的插件页面中打开 **MailRelay Guard - 邮件中心**。该页面由 AstrBot 的已登录 Dashboard 承载，不会额外启动公开的 Web 服务。
...
桌面端进入邮件中心后，左侧应用导航会切换为窄轨，列表控制默认只显示文件夹、搜索、数量和刷新；筛选与归档说明按需展开。选中邮件时，投递模式、时间、收件人状态和 SMTP 结果等技术信息默认按需展开，不会长期挤占预览区域。点击“专注阅读”可完全收起列表，让正文独占工作区；再次点击“显示列表”即可继续切换邮件。手机端仍会先显示列表；打开邮件后进入详情阅读，并可返回列表继续切换。
